...
# xhisper v2.0
# Push-to-talk dictation for Linux. Press and hold to record, release to transcribe.
...
STATUS_FILE="/tmp/xhisper.status"
LOCK_FILE="/tmp/xhisper.lock"
PROCESS_PATTERN="pw-record.*xhisper\.wav"
...
# Push-to-talk: check if recording is active
is_recording() {
...
acquire_lock() {
  # Create lock file and record PID
...
# Main - Push-to-talk behavior
# Single invocation: press key -> start recording, release key -> transcribe
acquire_lock

if is_recording; then
  # Recording in progress - stop and transcribe
  ...
  # No recording running - start recording
  rm -f "$RECORDING"
  rm -f "$STATUS_FILE"
