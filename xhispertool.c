...
int setup_uinput() {
    ...
    if (fd_uinput < 0) {
        ...
        fprintf(stderr, "\n");
        fprintf(stderr, "xhisper needs to create a virtual keyboard to type text.\n");
        fprintf(stderr, "If you see 'Permission denied', run:\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "  cd /path/to/xhisper\n");
        fprintf(stderr, "  sudo make install-setuid\n");
        fprintf(stderr, "\n");
        fprintf(stderr, "This only needs to be done once. It does NOT give xhisper root privileges\n");
        fprintf(stderr, "beyond what's needed for creating the virtual keyboard.\n");
        return -1;
    }
...
int run_daemon() {
    ...
    // Drop privileges - we only needed root to open /dev/uinput
    if (getuid() != 0) {
        setuid(getuid());
    }
